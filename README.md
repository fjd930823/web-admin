...
系统支持使用 Token 配置文件自动发帖到 Xiuno 论坛。

**🔥 核心特性：**
- ✅ **无需密码** - 前端只输入账号，使用配置文件中的 token
- ✅ **真正热更新** - 修改配置文件立即生效
- ✅ **批量发帖** - 支持一次为多个账号发帖
...
在浏览器中登录 Xiuno 论坛后，按 F12 打开开发者工具：

```
Application -> Cookies -> 找到 bbs_token
复制 Value 和 Expires 字段
```

#### 2. 配置 Token

编辑项目根目录的 `xiuno-tokens.conf` 文件：
...
系统会自动使用配置文件中对应的 Token 发帖，并自动检查过期时间。

### ✨ 特性

- ✅ **极简单** - 只需配置 token 和过期时间
- ✅ **极快速** - 发帖速度 < 1 秒
- ✅ **自动过期检查** - 发帖时自动检查 token 是否过期
- ✅ **零资源** - 几乎不消耗内存和 CPU
- ✅ **100% 成功率** - 使用真实的 token
- ✅ **多账号** - 支持配置无限多个账号
- ✅ **真正热更新** - 修改配置文件立即生效，无需重启或调用接口

### 📚 完整文档

查看详细使用指南：**[FINAL_USAGE.md](./FINAL_USAGE.md)**
...
# 显示帮助
node test-token-config.js --help
```

### 🔧 配置格式

**文件位置：** `项目根目录/xiuno-tokens.conf`
...
test@example.com=bbs_token=pqr456stu789vwx012
```
